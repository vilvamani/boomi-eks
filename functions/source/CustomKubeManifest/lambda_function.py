...
def create_handler(event, _):
    ...
    outp = run_command("kubectl create --validate=false --save-config -o json -f %s" % manifest_file)
...
def update_handler(event, _):
    ...
    if not manifest_file:
        return physical_resource_id
    outp = run_command("kubectl apply --validate=false -o json -f %s" % manifest_file)
    helper.Data = build_output(json.loads(outp))
    return helper.Data["selfLink"]
